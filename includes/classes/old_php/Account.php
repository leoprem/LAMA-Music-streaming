...
//            $hashPw = mysqli_fetch_assoc($query);  
            $en_pw = md5($pw);
            $pw_found = mysqli_query($this->con,"SELECT * FROM users WHERE username = '$un' AND password = '$en_pw'");
...
//            }
        ...
        {
//            $hashedPw = password_hash($pw,PASSWORD_DEFAULT);  #HASHING DOESNT WORK IN OLD VER. PHP
            $hashedPw = md5($pw);
